...
[[ ! -e "$STAGING" && ! -e "$FINAL" ]] || { echo "[FATAL] run id already exists: $RUN_ID" >&2; exit 2; }
mkdir -p "$STAGING/samples" "$STAGING/work"

NORMALIZED="$STAGING/work/manifest.tsv"
python3 "$SCRIPT_DIR/evidence/validate_manifest.py" --manifest "$MANIFEST" --out "$NORMALIZED"
mapfile -t SAMPLES < <(awk -F '\t' 'NR > 1 {print $2}' "$NORMALIZED")
[[ ${#SAMPLES[@]} -gt 0 ]] || { echo "[FATAL] batch has no samples" >&2; exit 2; }
BATCH_ID="$(awk -F '\t' 'NR == 2 {print $1}' "$NORMALIZED")"
...
if [[ ! -f "$STATE_FILE" ]]; then
  ...
  python3 "$SCRIPT_DIR/evidence/run_state.py" --state "$STATE_FILE" status --value running
fi
...
while IFS=$'\t' read -r batch_id sample_id role library_mode umi_mode r1 r2 expected_target; do
  ...
  SAMPLE_R1="$r1" SAMPLE_R2="$r2" EVIDENCE_LIBRARY_MODE="$library_mode" EVIDENCE_UMI_MODE="$umi_mode" \
    ...
    "$SCRIPT_DIR/20_run_pipeline.sh" --sample "$sample_id" --threads "$THREADS" --evidence-v2 --evidence-config "$CONFIG"
...
python3 "$SCRIPT_DIR/evidence/collect_batch_metrics.py" --manifest "$NORMALIZED" --repo-root "$REPO_ROOT" --run-map "$RUN_MAP" --out "$METRICS"
...
python3 "$SCRIPT_DIR/evidence/apply_control_status.py" --statuses "$STAGING/control_status.tsv" --evidence-root "$STAGING"
