...
[[ ! -e "$OUTDIR" && ! -e "$FINAL_DIR" ]] || { echo "[FATAL] run id already exists: $RUN_ID" >&2; exit 2; }
mkdir -p "$OUTDIR"
...
if [[ -n "$COMPOSITE_DB" ]]; then
  ...
  python3 "$EVIDENCE_DIR/route_queries.py" --fasta "$QUERIES" --out-dir "$TMPDIR_RUN/routes" > "$OUTDIR/query_routing.tsv.tmp"
  mv -f "$OUTDIR/query_routing.tsv.tmp" "$OUTDIR/query_routing.tsv"
  SHORT_RAW="$TMPDIR_RUN/blastn-short.tsv"; BLASTN_RAW="$TMPDIR_RUN/blastn.tsv"; : > "$SHORT_RAW"; : > "$BLASTN_RAW"
  run_blast() {
    local task="$1" query="$2" output="$3" reward="$4" penalty="$5" word_size="$6"
    [[ -s "$query" ]] || return 0
    blastn -task "$task" -query "$query" -db "$COMPOSITE_DB" \
      -outfmt '6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen' \
      -word_size "$word_size" -reward "$reward" -penalty "$penalty" -gapopen 5 -gapextend 2 \
      -dust "$DUST" -soft_masking "$SOFT_MASKING" -max_target_seqs 50 -evalue 1000 -num_threads "$THREADS" >> "$output"
  }
  run_blast blastn-short "$TMPDIR_RUN/routes/queries_lt30.fa" "$SHORT_RAW" 1 -3 7
  run_blast blastn-short "$TMPDIR_RUN/routes/queries_30to49.fa" "$SHORT_RAW" 1 -3 7
  run_blast blastn "$TMPDIR_RUN/routes/queries_30to49.fa" "$BLASTN_RAW" 2 -3 11
  run_blast blastn "$TMPDIR_RUN/routes/queries_ge50.fa" "$BLASTN_RAW" 2 -3 11
...
if [[ -n "$PANEL_INDEX" && -n "$PANEL_FASTA" && -n "$R1" && -n "$R2" ]]; then
  "$EVIDENCE_DIR/map_read_support.sh" --sample "$SAMPLE" --index "$PANEL_INDEX" --reference "$PANEL_FASTA" \
    ...
    --minimum-mapq "$MIN_MAPQ" --minimum-base-quality "$MIN_BASEQ" --window-bp "$WINDOW_BP" --threads "$THREADS"
...
python3 "$EVIDENCE_DIR/classify_sample.py" --sample "$SAMPLE" --loci "$OUTDIR/locus_evidence.tsv" \
  ...
  --provenance "$OUTDIR/provenance.json" --out "$OUTDIR/sample_evidence.json"
...
for artifact in fragment_evidence locus_evidence competitive_hits read_support coverage sample_evidence provenance evidence_report runtime_preflight; do
  case "$artifact" in
    sample_evidence|provenance|runtime_preflight) extension=json ;;
